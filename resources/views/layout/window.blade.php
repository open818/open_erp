                ...
                Init:function(){
                    $(".tabItemContainer>li").click(function(){
                        ...
                        if(parents.length > 0){
                            ...
                            if(curr_tab.loaded == '1' && curr_tab.parents_value == v){
                                return ;
                            }
                        ...
                        $('#table_'+curr_tab.table_id).setGridParam({
                            url: '/_develop/getTableDate/'+curr_tab.tab_id+'/'+curr_tab.table_id,
                            postData: select_kv,
                            datatype:'json'
                        ...
                        <div class="col-xs-1 tabItemContainer" style="padding: 0;">
                            ...
                            @foreach($tabs as $tab)
                                <li data-tab_id="{{$tab->AD_TAB_ID}}"
                                    ...
                                    data-table_id_name="{{strtoupper($tab->id_name)}}"
                                    data-table_parent="@if(!empty($tab->parent_names)){{strtoupper(implode(',',$tab->parent_names))}}@endif"
                                ...
                                <div class="tabBodyItem @if($i==1) tabBodyCurrent @endif">
                                    ...
                                    <script type="application/javascript">
                                        $("#table_{{$tab->AD_TAB_ID}}").jqGrid({
                                            @if($i == 1)
                                            url:"/_develop/getTableDate/{{$tab->AD_TAB_ID}}/{{$tab->AD_TAB_ID}}",
                                            @endif
                                            datatype : "json",
                                            mtype: 'GET',
                                            rowNum: '-1',
                                            ...
                                            colModel:[
                                                ...
                                                {
                                                    name : '{{strtoupper($field->COLUMNNAME)}}',
                                                    label: '{{$field->NAME}}',
                                                    index : '{{strtoupper($field->COLUMNNAME)}}',
                                                    sortable: true,
                                                    sort: ''
                                                },
                                                @endforeach
